...
// Функция для преобразования Excel даты в формат YYYY-MM-DD
function parseExcelDate(excelDate) {
  if (typeof excelDate === "number") {
    const date = XLSX.SSF.parse_date_code(excelDate);
    return `${date.y}-${String(date.m).padStart(2, "0")}-${String(
...
function updateTable(data) {
  ...
  data.forEach((student, index) => {
    ...
    cells.forEach((cellValue, i) => {
      ...
      // Добавление возможности редактировать ячейки
      if (i > 0) {
        // Пропускаем №
        cell.classList.add("editable");
        cell.addEventListener("click", () => {
          const newValue = prompt("Измените значение:", cellValue);
          if (newValue !== null) {
            // Обновляем данные в JSON
            student[Object.keys(student)[i - 1]] = newValue;

            // Обновляем таблицу с изменениями
            updateTable(data);
          }
        });
      }
